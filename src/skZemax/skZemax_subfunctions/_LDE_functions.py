...
def _convert_raw_surface_input_(
    self, in_surface: int | ZOSAPI_Editors_LDE_ILDERow, return_index: bool = True
) -> int | ZOSAPI_Editors_LDE_ILDERow:
    return _convert_raw_input_worker_(
        self,
        in_value=in_surface,
        object_type=self.ZOSAPI.Editors.LDE.ILDERow,
...
def LDE_GetSurface(self, SurfaceNum: int) -> ZOSAPI_Editors_LDE_ILDERow:
    ...
    if self._verbose:
        cp(
            f"!@ly!@LDE_GetSurface :: Asked for Surface [!@lm!@{SurfaceNum}!@ly!@] but there are only !@lm!@{self.LDE_GetNumberOfSurfaces()}!@ly!@ surfaces built up to index !@lm!@{self.LDE_GetNumberOfSurfaces() - 1}!@ly!@."
        )
    return None


def LDE_InsertNewSurface(
    self, insertSurface: int | ZOSAPI_Editors_LDE_ILDERow
    ...
    return self.TheSystem.LDE.InsertNewSurfaceAt(
        self._convert_raw_surface_input_(insertSurface, return_index=True)
    )
...
def LDE_RemoveSurface(self, delSurface: int | ZOSAPI_Editors_LDE_ILDERow) -> None:
    """
    ...
    self.TheSystem.LDE.RemoveSurfaceAt(
        self._convert_raw_surface_input_(delSurface, return_index=True)
    )


def LDE_CopyAndInsertSurfacesFromFile(
    self,
    in_Surface: int | ZOSAPI_Editors_LDE_ILDERow,
    path_to_file: str,
    first_surface_to_copy: int,
    number_surfaces_to_copy: int,
) -> None:
    """
    ...
    self.TheSystem.LDE.CopySurfacesFrom(
        new_system.LDE, first_surface_to_copy, number_surfaces_to_copy, SurfaceLDE
    )
    self.TheApplication.CloseSystemAt(
        self.TheApplication.get_NumberOfOpticalSystems() - 1, False
    )
    del new_system
...
def LDE_GetNamesOfAllSurfaceTypes(self, print_to_console: bool = False) -> list:
    ...
    :rtype: list
    """
    surface_types = __LowLevelZemaxStringCheck__(
        self, in_obj=self.ZOSAPI.Editors.LDE.SurfaceType
    )
    if print_to_console:
...
def LDE_ChangeSurfaceType(
    self, in_Surface: int | ZOSAPI_Editors_LDE_ILDERow, surface_type: str
) -> ZOSAPI_Editors_LDE_ILDERow:
    """
    ...
    surfacetype = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Editors.LDE.SurfaceType, str(surface_type)
    )
    if surfacetype is not None:
        SurfaceLDE.ChangeType(SurfaceLDE.GetSurfaceTypeSettings(surfacetype))
    elif self._verbose:
        cp("!@ly!@LDE_ChangeSurfaceType :: Did not change surface type")
    return SurfaceLDE
...
def LDE_GetNamesOfAllApertureTypes(self, print_to_console: bool = False) -> list:
    ...
    :rtype: list
    """
    aperture_types = __LowLevelZemaxStringCheck__(
        self, in_obj=self.ZOSAPI.Editors.LDE.SurfaceApertureTypes
    )
    if print_to_console:
...
def LDE_GetSurfaceApertureType(
    ...
    """
    return str(
        self._convert_raw_surface_input_(
            in_Surface, return_index=False
        ).ApertureData.CurrentType
    )


def LDE_GetApertureTypeSettings(
    self, in_Surface: int | ZOSAPI_Editors_LDE_ILDERow, aperture_type: str
    ...
    """
    SurfaceLDE = self._convert_raw_surface_input_(in_Surface, return_index=False)
    aperturetype = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Editors.LDE.SurfaceApertureTypes, str(aperture_type)
    )
    settings = None
    if aperturetype is not None:
        ...
        cp(
            f"!@ly!@LDE_GetApertureTypeSettings :: Did not find {aperture_type!s} aperture type settings"
        )
        return None
    return settings
...
def LDE_ChangeApertureToRectangular(
    self,
    in_Surface: int | ZOSAPI_Editors_LDE_ILDERow,
    XHalfWidth: float = 10.0,
    YHalfWidth: float = 10.0,
    ApertureXDecenter: float = 0.0,
    ApertureYDecenter: float = 0.0,
) -> ZOSAPI_Editors_LDE_ILDERow:
    """
...
def LDE_ChangeApertureToCircular(
    self,
    in_Surface: int | ZOSAPI_Editors_LDE_ILDERow,
    MinimumRadius: float = 0.0,
    MaximumRadius: float = 10.0,
    ApertureXDecenter: float = 0.0,
    ApertureYDecenter: float = 0.0,
) -> ZOSAPI_Editors_LDE_ILDERow:
    """
...
def LDE_ChangeApertureToCircularObscuration(
    self,
    in_Surface: int | ZOSAPI_Editors_LDE_ILDERow,
    MinimumRadius: float = 0.0,
    MaximumRadius: float = 10.0,
...
def LDE_ChangeApertureToFloating(
    self, in_Surface: int | ZOSAPI_Editors_LDE_ILDERow
) -> ZOSAPI_Editors_LDE_ILDERow:
    """
    ...
    SurfaceLDE = self._convert_raw_surface_input_(in_Surface, return_index=False)
    settings = self.LDE_GetApertureTypeSettings(
        in_Surface=SurfaceLDE, aperture_type="FloatingAperture"
    )
    SurfaceLDE.ApertureData.ChangeApertureTypeSettings(settings)
    return SurfaceLDE


def LDE_SetSurfaceAsStop(self, in_Surface: int | ZOSAPI_Editors_LDE_ILDERow) -> None:
    """
...
def LDE_GetStopSurface(self) -> ZOSAPI_Editors_LDE_ILDERow:
    ...
    :rtype: ZOSAPI_Editors_LDE_ILDERow
    """
    return self.LDE_GetSurface(
        int(
            np.argmax(
                [
                    self.LDE_CheckIfSurfaceIsStop(x)
                    for x in range(self.LDE_GetNumberOfSurfaces())
                ]
            )
        )
    )
...
def _LDE_GetSurfaceColumns_(self) -> tuple:
    ...
    :rtype: tuple
    """
    all_surface_columns = __LowLevelZemaxStringCheck__(
        self, self.ZOSAPI.Editors.LDE.SurfaceColumn
    )
    ...
    surface_column_calls = [
        getattr(self.ZOSAPI.Editors.LDE.SurfaceColumn, x) for x in surface_columns
    ]
...
def LDE_GetSurfaceColumnEnum(
    self, in_str: str, in_Surface: None | int | ZOSAPI_Editors_LDE_ILDERow = None
    ...
    if np.any(bool_mask):
        return self._CheckIfStringValidInDir_(
            ...
            surface_columns[int(np.where(bool_mask)[0][0])],
        )
    if self._verbose:
        cp(
            f"!@ly!@LDE_GetSurfaceColumnEnum :: Did not find [!@lm!@{in_str}!@ly!@] in surface properties."
        )
    return None
...
def LDE_SetTiltDecenterOfSurface(
    self,
    in_Surface: int | ZOSAPI_Editors_LDE_ILDERow,
    BeforeSurfaceDecenterX: float = 0.0,
    BeforeSurfaceDecenterY=0.0,
    BeforeSurfaceTiltX: float = 0.0,
    BeforeSurfaceTiltY: float = 0.0,
    BeforeSurfaceTiltZ: float = 0.0,
    AfterSurfaceDecenterX: float = 0.0,
    AfterSurfaceDecenterY=0.0,
    AfterSurfaceTiltX: float = 0.0,
    AfterSurfaceTiltY: float = 0.0,
    AfterSurfaceTiltZ: float = 0.0,
    BeforeSurfaceOrder: str = "Decenter_Tilt",
    AfterSurfaceOrder: str = "Decenter_Tilt",
) -> ZOSAPI_Editors_LDE_ILDERow:
    """
    ...
    """
    SurfaceLDE = self._convert_raw_surface_input_(in_Surface, return_index=False)
    SurfaceLDE.TiltDecenterData.BeforeSurfaceOrder = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Editors.LDE.TiltDecenterOrderType, BeforeSurfaceOrder
    )
    SurfaceLDE.TiltDecenterData.AfterSurfaceOrder = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Editors.LDE.TiltDecenterOrderType, AfterSurfaceOrder
    )
...
def LDE_SetTiltDecenterAfterSurfaceMode(
    self, in_Surface: int | ZOSAPI_Editors_LDE_ILDERow, mode: str | tuple[str, int]
) -> ZOSAPI_Editors_LDE_ILDERow:
    """
    ...
    if isinstance(mode, str):
        if "explicit" in mode.lower():
            ...
            cp(
                f"!@ly!@LDE_SetTiltDecenterAfterSurfaceMode :: mode [!@lm!@{mode!s}!@ly!@] not understood."
            )
    elif isinstance(mode, tuple):
        try:
            if "pickup" in str(mode[0]).lower():
                SurfaceLDE.TiltDecenterData.SetAfterSurfaceModePickup(
                    self._convert_raw_surface_input_(mode[1], return_index=True)
                )
            elif "reverse" in str(mode[0]).lower():
                SurfaceLDE.TiltDecenterData.SetAfterSurfaceModeReverse(
                    self._convert_raw_surface_input_(mode[1], return_index=True)
                )
            else:
                cp(
                    f"!@ly!@LDE_SetTiltDecenterAfterSurfaceMode :: mode [!@lm!@{mode[0]!s}!@ly!@] not understood."
                )
        except Exception:
            cp(
                f"!@ly!@LDE_SetTiltDecenterAfterSurfaceMode :: Problem setting input [!@lm!@{mode!s}!@ly!@]."
            )
    else:
        cp(
            f"!@ly!@LDE_SetTiltDecenterAfterSurfaceMode :: Expected [!@lm!@string!@ly!@] or [!@lm!@tuple!@ly!@] as input. Got [!@lm!@{type(mode)!s}!@ly!@]."
        )


def LDE_GetObjectRotationAndPositionMatrices(
    self, in_Surface: int | ZOSAPI_Editors_LDE_ILDERow
    ...
    R = np.array(
        self.MFE_GetOperandValues(
            "GLCR", np.array([[in_Surface, x + 1, 0, 0, 0, 0, 0, 0] for x in range(9)])
        ).reshape(3, 3)
    )
    return np.array(
        [
            global_transofmrations[-3],
            global_transofmrations[-2],
            global_transofmrations[-1],
        ]
    ), R
...
def LDE_RunRayTrace(self, ray_trace_rays: xr.Dataset = None) -> xr.Dataset:
    ...
    desired_ray_trace_call = _CheckIfStringValidInDir_(
        self,
        opened_batch_ray_trace,
        ray_trace_rays.attrs["ray_trace_type"],
        extra_include_filter=["Create"],
        extra_exclude_filter=["NSC"],
    )
    if "CreateNormUnpol" in str(desired_ray_trace_call):
        ray_trace_rays = self._run_NormUnPol_raytrace_(
            opened_batch_ray_trace, desired_ray_trace_call, ray_trace_rays
        )
    opened_batch_ray_trace.Close()
    return ray_trace_rays


def LDE_BuildRayTraceNormalizedUnpolarizedRays(
    self,
    ...
    wavelengths: int
    | ZOSAPI_SystemData_IWavelength
    | list[int, ZOSAPI_SystemData_IWavelength]
    | np.ndarray[int, ZOSAPI_SystemData_IWavelength] = None,
    ...
    if ending_surface is None:
        ...
        ending_surface = self._convert_raw_surface_input_(
            ending_surface, return_index=True
        )
    if wavelengths is None:
        wavelengths_idx = np.array(
            [self.Wavelength_GetPrimaryWavelength().WavelengthNumber]
        )
    else:
        if not isinstance(wavelengths, np.ndarray) and not isinstance(
            wavelengths, list
        ):
            wavelengths = np.array([wavelengths])
        wavelengths_idx = np.array(
            [
                _convert_raw_wavelength_input_(self, x, return_index=True)
    ...
    return xr.Dataset(
        ...
        attrs={
            ...
            "ray_type": str(
                _CheckIfStringValidInDir_(
                    self, self.ZOSAPI.Tools.RayTrace.RaysType, ray_type
                )
            ),
            "OPD_mode": str(
                _CheckIfStringValidInDir_(
                    self, self.ZOSAPI.Tools.RayTrace.OPDMode, OPD_mode
                )
            ),
        },
    )
...
def _run_NormUnPol_raytrace_(
    self,
    opened_batch_ray_trace: ZOSAPI_Tools_RayTrace_IBatchRayTrace,
    desired_ray_trace_call: CLR_MethodBinding,
    ray_trace_rays: xr.Dataset,
):
    """
    ...
    if bool(int(ray_trace_rays.attrs["do_all_surfaces_to_ending"])):
        surfaces_to_trace = np.arange(
            0, int(ray_trace_rays.attrs["ending_surface"]) + 1
        )
    else:
    ...
    if bool(int(ray_trace_rays.attrs["trace_wavelengths_individually"])):
        ...
        blank_rays = np.zeros(
            (
                ray_trace_rays.wavelengths_idx.shape[0],
    ...
    ray_trace_rays = ray_trace_rays.assign_coords(
        {"surf": (("surf"), surfaces_to_trace.astype(int))}
    )
    ray_trace_rays = ray_trace_rays.assign(
        {"error": (eval(dims), blank_rays.astype(bool))}
    )
    ray_trace_rays = ray_trace_rays.assign(
        {"vignette": (eval(dims), blank_rays.astype(bool))}
    )
    ray_trace_rays = ray_trace_rays.assign(
    ...
    ray_trace_rays = ray_trace_rays.assign(
        {"Ycosine": (eval(dims), blank_rays.astype(float))}
    )
    ray_trace_rays = ray_trace_rays.assign(
        {"Zcosine": (eval(dims), blank_rays.astype(float))}
    )
    ray_trace_rays = ray_trace_rays.assign(
        {"Xnormal": (eval(dims), blank_rays.astype(float))}
    )
    ray_trace_rays = ray_trace_rays.assign(
        {"Ynormal": (eval(dims), blank_rays.astype(float))}
    )
    ray_trace_rays = ray_trace_rays.assign(
    ...
    ray_trace_rays = ray_trace_rays.assign(
        {
            "surface_comment": (
                ...
                np.array(
                    [
                        str(self.LDE_GetSurface(x).Comment)
                        for x in range(surfaces_to_trace.shape[0])
                    ]
                ),
            )
        }
        ...
        for surf_idx, surf in enumerate(surfaces_to_trace):
            ray_tracer = desired_ray_trace_call(
                ...
                _CheckIfStringValidInDir_(
                    ...
                    ray_trace_rays.attrs["ray_type"],
                ),
                int(surf),
            )
            dataReader = self.BatchRayTrace.ReadNormUnpolData(
                opened_batch_ray_trace, ray_tracer
            )
            ...
            while not isFinished and rayData is not None:
                ...
                if readSegments == 0:
                    ...
                    ray_trace_rays.error.values[surf_idx, :] = _ctype_to_numpy_(
                        self,
                        rayData.errorCode,
                        data_length=readSegments,
                        data_type=np.int32,
                    ).astype(bool)
                    ray_trace_rays.vignette.values[surf_idx, :] = _ctype_to_numpy_(
                        self,
                        rayData.errorCode,
                        data_length=readSegments,
                        data_type=np.int32,
                    ).astype(bool)
                    ray_trace_rays.X.values[surf_idx, :] = _ctype_to_numpy_(
                        self, rayData.X, data_length=readSegments, data_type=np.double
                    )
                    ray_trace_rays.Y.values[surf_idx, :] = _ctype_to_numpy_(
                        self, rayData.Y, data_length=readSegments, data_type=np.double
                    )
                    ray_trace_rays.Z.values[surf_idx, :] = _ctype_to_numpy_(
                        self, rayData.Z, data_length=readSegments, data_type=np.double
                    )
                    ray_trace_rays.Xcosine.values[surf_idx, :] = _ctype_to_numpy_(
                        self, rayData.L, data_length=readSegments, data_type=np.double
                    )
                    ray_trace_rays.Ycosine.values[surf_idx, :] = _ctype_to_numpy_(
                        self, rayData.M, data_length=readSegments, data_type=np.double
                    )
                    ray_trace_rays.Zcosine.values[surf_idx, :] = _ctype_to_numpy_(
                        self, rayData.N, data_length=readSegments, data_type=np.double
                    )
                    ray_trace_rays.Xnormal.values[surf_idx, :] = _ctype_to_numpy_(
                        self, rayData.l2, data_length=readSegments, data_type=np.double
                    )
                    ray_trace_rays.Ynormal.values[surf_idx, :] = _ctype_to_numpy_(
                        self, rayData.m2, data_length=readSegments, data_type=np.double
                    )
                    ray_trace_rays.Znormal.values[surf_idx, :] = _ctype_to_numpy_(
                        self, rayData.n2, data_length=readSegments, data_type=np.double
                    )
                    ray_trace_rays.OPD.values[surf_idx, :] = _ctype_to_numpy_(
                        self, rayData.opd, data_length=readSegments, data_type=np.double
                    )
                    ray_trace_rays.intensity.values[surf_idx, :] = _ctype_to_numpy_(
                        self,
                        rayData.intensity,
                        data_length=readSegments,
                        data_type=np.double,
            ...
            for surf_idx, surf in enumerate(surfaces_to_trace):
                ...
                while not isFinished and rayData is not None:
                    ...
                    if readSegments == 0:
                        ...
                        totalSegRead = totalSegRead + readSegments
                        ray_trace_rays.error.values[wi, surf_idx, :] = _ctype_to_numpy_(
                            self,
                            rayData.errorCode,
                            data_length=readSegments,
                            data_type=np.int32,
                        ).astype(bool)
                        ray_trace_rays.vignette.values[wi, surf_idx, :] = (
                            _ctype_to_numpy_(
                                self,
                                rayData.errorCode,
                                data_length=readSegments,
                        ...
                        ray_trace_rays.X.values[wi, surf_idx, :] = _ctype_to_numpy_(
                            self,
                            rayData.X,
                            data_length=readSegments,
                            data_type=np.double,
                        )
                        ray_trace_rays.Y.values[wi, surf_idx, :] = _ctype_to_numpy_(
                            self,
                            rayData.Y,
                            data_length=readSegments,
                            data_type=np.double,
                        )
                        ray_trace_rays.Z.values[wi, surf_idx, :] = _ctype_to_numpy_(
                            self,
                            rayData.Z,
                            data_length=readSegments,
                            data_type=np.double,
                        ...
                        ray_trace_rays.OPD.values[wi, surf_idx, :] = _ctype_to_numpy_(
                            self,
                            rayData.opd,
                            data_length=readSegments,
                            data_type=np.double,
    ...
    ray_trace_rays.intensity.values = (
        ray_trace_rays.intensity / ray_trace_rays.pupil_apodization
    ).values
    # Add global system variables
    global_transofmrations = np.array(
        [self.TheSystem.LDE.GetGlobalMatrix(int(x)) for x in ray_trace_rays.surf.values]
    )
    # After comapring with Zemax itself, GetGlobalMatrix() seems to return bad R coefficent values. Offsets seem okay still.
    ...
    R = np.array(
        [
            self.MFE_GetOperandValues(
                "GLCR", np.array([[s, x + 1, 0, 0, 0, 0, 0, 0] for x in range(9)])
            ).reshape(3, 3)
            for s in ray_trace_rays.surf.values
        ]
    )
    transformer = xr.Dataset(
        {
            ...
            "Vcosine": (
                ...
                np.array(
                    [
                        ray_trace_rays.Xcosine.values,
                        ray_trace_rays.Ycosine.values,
                        ray_trace_rays.Zcosine.values,
                    ]
                ),
            ),
            "Vnormal": (
                ...
                np.array(
                    [
                        ray_trace_rays.Xnormal.values,
                        ray_trace_rays.Ynormal.values,
                        ray_trace_rays.Znormal.values,
                    ]
                ),
            ),
        }
    ...
    ray_trace_rays = ray_trace_rays.assign(
        {
            "Xcosine_global": (
                eval(dims),
                np.swapaxes(cosine_glo[:, 0, :, :].values, 0, 1),
            )
        }
    )
    ray_trace_rays = ray_trace_rays.assign(
        {
            "Ycosine_global": (
                eval(dims),
                np.swapaxes(cosine_glo[:, 1, :, :].values, 0, 1),
            )
        }
    )
    ray_trace_rays = ray_trace_rays.assign(
        {
            "Zcosine_global": (
                eval(dims),
                np.swapaxes(cosine_glo[:, 2, :, :].values, 0, 1),
            )
        }
    )
    ray_trace_rays = ray_trace_rays.assign(
        {
            "Xnormal_global": (
                eval(dims),
                np.swapaxes(normal_glo[:, 0, :, :].values, 0, 1),
            )
        }
    )
    ray_trace_rays = ray_trace_rays.assign(
        {
            "Ynormal_global": (
                eval(dims),
                np.swapaxes(normal_glo[:, 1, :, :].values, 0, 1),
            )
        }
    )
    ray_trace_rays = ray_trace_rays.assign(
        {
            "Znormal_global": (
                eval(dims),
                np.swapaxes(normal_glo[:, 2, :, :].values, 0, 1),
            )
        }
    )
    # Find "Angle in".
    ray_trace_rays.angle_in.values = np.rad2deg(
        np.arccos(
            np.abs(
                ray_trace_rays.Xcosine.roll(surf=1) * ray_trace_rays.Xnormal
                + ray_trace_rays.Ycosine.roll(surf=1) * ray_trace_rays.Ynormal
                + ray_trace_rays.Zcosine.roll(surf=1) * ray_trace_rays.Znormal
            )
        )
    ).values
