from skZemax.skZemax_subfunctions._ZOSAPI_interface_functions import __LowLevelZemaxStringCheck__, _convert_raw_input_worker_
...
def _convert_raw_operand_input_(self, in_obj: Union[int, ZOSAPI_Editors_MFE_IMFERow], return_index:bool=True)->Union[int, ZOSAPI_Editors_MFE_IMFERow]:
    return _convert_raw_input_worker_(self, in_value=in_obj, object_type=self.ZOSAPI.Editors.MFE.IMFERow, return_index=return_index)
...
def MFE_GetOperand(self, operandNum: int)->ZOSAPI_Editors_MFE_IMFERow:
    ...
    if self._verbose: cp('!@ly!@MFE_GetOperand :: Asked for Operand [!@lm!@{}!@ly!@] but there are only !@lm!@{}!@ly!@ operands built.'.format(operandNum, self.MFE_GetNumberOfOperands()))
    return None
...
def MFE_InsertNewOperand(self, newOperand: Union[int, ZOSAPI_Editors_MFE_IMFERow])->None:
    """
    ...
    :rtype: _type_
    """
    return self.TheSystem.MFE.InsertNewOperandAt(self._convert_raw_operand_input_(newOperand, return_index=True))

def MFE_SetOperand(self, in_Operand: Union[int, ZOSAPI_Editors_MFE_IMFERow],
                   operand_type:str,
                   target   : Union[int,str,float]=None,
                   weight   : Union[int,str,float]=None,
                   param1   : Union[int,str,float]=None,
                   param2   : Union[int,str,float]=None,
                   param3   : Union[int,str,float]=None,
                   param4   : Union[int,str,float]=None,
                   param5   : Union[int,str,float]=None,
                   param6   : Union[int,str,float]=None,
                   param7   : Union[int,str,float]=None,
                   param8   : Union[int,str,float]=None,
                   comment  : Union[int,str,float]=None,
                   value    : Union[int,str,float]=None,
                   contrib  : Union[int,str,float]=None,
                   )->None:
    """
        ...
        if invalue is not None:
            if isinstance(invalue, int):
                try:
                    ...
                    in_Operand.GetCellAt(in_cell).Value = str(invalue) # If fail, fall back to string input.
            else:
                in_Operand.GetCellAt(in_cell).Value = str(invalue)
    in_Operand = self._convert_raw_operand_input_(in_Operand, return_index=False)
    in_Operand.ChangeType(self._CheckIfStringValidInDir_(self.ZOSAPI.Editors.MFE.MeritOperandType, operand_type, check_if_upper=True))
    _assign_(comment, 1)
    _assign_(param1, 2)
    _assign_(param2, 3)
    _assign_(param3, 4)
...
def MFE_GetOperandValues(self,
                         operand_type:str,
                         param_array: np.ndarray)->np.ndarray:
    """
    ...
    if param_array.shape[1] != 8:
        cp('!@ly!@MFE_GetOperandValues :: Expected 8 parameters for each configuration along the first dimension of given array. Found [!@lm!@%i!@lg!@] instead.' % param_array.shape[1])
        return None
    out = np.zeros(param_array.shape[0])*np.nan
    for idx, config in enumerate(param_array):
        out[idx] = self.TheSystem.MFE.GetOperandValue(self._CheckIfStringValidInDir_(self.ZOSAPI.Editors.MFE.MeritOperandType, operand_type, check_if_upper=True),
                                                       int(config[0]), int(config[1]), float(config[2]),
                                                       float(config[3]), float(config[4]), float(config[5]),
                                                       float(config[6]), float(config[7]))
    return out
