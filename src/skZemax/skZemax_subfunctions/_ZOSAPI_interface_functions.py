...
def _convert_raw_input_worker_(
    self, in_value: int | Any, object_type: Any, return_index: bool = True
) -> int | Any:
    """
    ...
    def _object_to_index_tree_() -> int:
        ...
        if isinstance(in_value, self.ZOSAPI.Editors.LDE.ILDERow):
            return int(in_value.SurfaceNumber)
        if isinstance(in_value, self.ZOSAPI.SystemData.IWavelength):
            return int(in_value.WavelengthNumber)
        if isinstance(
            in_value, (self.ZOSAPI.Editors.MFE.IMFERow, self.ZOSAPI.Editors.MCE.IMCERow)
        ):
            return int(in_value.OperandNumber)
        if isinstance(in_value, self.ZOSAPI.Editors.NCE.INCERow):
            return int(in_value.RowIndex + 1)  # Indexed from zero (unlike surfaces)
        cp(
            f"!@ly!@_convert_raw_input_worker_ :: [!@lm!@{object_type}!@ly!@] not found."
        )
        return None
    ...
    def _index_to_object_tree_() -> Any:
        ...
        if str(object_type) == str(self.ZOSAPI.Editors.LDE.ILDERow):
            return self.LDE_GetSurface(int(in_value))
        if str(object_type) == str(self.ZOSAPI.SystemData.IWavelength):
            return self.Wavelength_GetWavelength(int(in_value))
        if str(object_type) == str(self.ZOSAPI.Editors.MFE.IMFERow):
            return self.MFE_GetOperand(int(in_value))
        if str(object_type) == str(self.ZOSAPI.Editors.MCE.IMCERow):
            return self.MCE_GetConfigOperand(int(in_value))
        if str(object_type) == str(self.ZOSAPI.Editors.NCE.INCERow):
    ...
    if return_index and not isinstance(in_value, object_type):
        return int(in_value)
    if return_index and isinstance(in_value, object_type):
        return _object_to_index_tree_()
    if not return_index and not isinstance(in_value, object_type):
