...
CPRINT_COMMAND_CONVERSIONS = {'bold'       : 1,   'dim'              : 2,   'underl'      : 4,   'blink'        : 5,
                              'invert'     : 7,   'hide'             : 8,   'clear'       : 0,   'clear_bold'   : 21,
                              'clear_dim'  : 22,  'clear_underline'  : 24,  'clear_blink' : 25,  'clear_invert' : 27,
                              'clear_hide' : 28,  'de'               : 39,  'w'           : 97,  'k'            : 30,
                              'r'          : 31,  'g'                : 32,  'y'           : 33,  'b'            : 34,
                              'm'          : 35,  'c'                : 36,  'lgr'         : 37,  'dgr'          : 90,
                              'lr'         : 91,  'lg'               : 92,  'ly'          : 93,  'lb'           : 94,
                              'lm'         : 95,  'lc'               : 96,  'bkde'        : 49,  'bkw'          : 107,
                              'bkk'        : 40,  'bkr'              : 41,  'bkg'         : 42,  'bky'          : 43,
                              'bkb'        : 44,  'bkm'              : 45,  'bkc'         : 46,  'bklgr'        : 47,
                              'bkdgr'      : 100, 'bklr'             : 101, 'bklg'        : 102, 'bkly'         : 103,
                              'bklb'       : 104, 'bklm'             : 105, 'bklc'        : 106}

def c_print(text: str,
            end: str='\n',
            flush: bool=False,
            command_marker: str='!@',
    ...
    split_text = text.split(command_marker)  # Split will always put commands on odd index as long as
    ...
    for idx in range(1, len(split_text), 2):  # Loop through all command markers.
        if idx == len(split_text) - 2:        # Check only the last set of commands for the !@keep!@ command.
            for split_text_item in split_text:
                if split_text_item.lower() == 'keep':
                    found_keep = True
                    break
        split_text[idx] = __get_sequence__(split_text[idx].split(' '))  # Replace all written commands within
...
def __get_sequence__(options: list=[]) -> str:
    """
    ...
    for idx in range(len(options)):
        command_sequence += str(CPRINT_COMMAND_CONVERSIONS.get(options[idx].lower(), ''))
