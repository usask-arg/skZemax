...
def RayAiming_GetNamesOfAllAimingMethods(self, print_to_console: bool = False) -> list:
    ...
    :rtype: list
    """
    available_aiming_methods = __LowLevelZemaxStringCheck__(
        self, in_obj=self.ZOSAPI.SystemData.RayAimingMethod
    )
    if print_to_console:
        cp(
...
def RayAiming_SetAimingMethod(self, method: str = "Off"):
    ...
    """
    available_aiming_methods = self.RayAiming_GetNamesOfAllAimingMethods(
        print_to_console=False
    )
    bool_mask = [str(method).lower() in x.lower() for x in available_aiming_methods]
    if np.any(bool_mask):
        self.TheSystem.SystemData.RayAiming.RayAiming = self._CheckIfStringValidInDir_(
            self.ZOSAPI.SystemData.RayAimingMethod,
            str(available_aiming_methods[int(np.where(bool_mask)[0][0])]),
        )
    else:
        cp(
            f"!@ly!@RayAiming_SetAimingMethod :: Ray aiming method [!@lm!@{method}!@ly!@] not found."
        )


def RayAiming_GetNamesOfAllAimingProperties(
    self, print_to_console: bool = False
) -> list:
    """
    ...
    :rtype: list
    """
    available_aiming_properties = [
        x
        for x in __LowLevelZemaxStringCheck__(
            self, self.TheSystem.SystemData.RayAiming, extra_exclude_filter="_"
        )
        if ("Method" not in x) and (x != "RayAiming")
    ]
    if print_to_console:
        cp(
...
def RayAiming_SetAimingProperty(
    self,
    aiming_property: str = "UseRayAimingCache",
    aiming_value: float | int | bool = True,
):
    """
    ...
    """
    available_aiming_props = self.RayAiming_GetNamesOfAllAimingProperties(
        print_to_console=False
    )
    bool_mask = [
        str(aiming_property).lower() in x.lower() for x in available_aiming_props
    ]
    if np.any(bool_mask):
        _SetAttrByStringIfValid_(
            self,
            self.TheSystem.SystemData.RayAiming,
            str(available_aiming_props[int(np.where(bool_mask)[0][0])]),
            aiming_value,
        )
    else:
        cp(
            f"!@ly!@RayAiming_SetAimingProperty :: Ray aiming property [!@lm!@{aiming_property}!@ly!@] not found."
        )
