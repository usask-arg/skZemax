...
def Solver_LocalOptimization(
    self, use_DampedLeastSquares: bool = True, numCores: int | None = None
) -> None:
    """
    ...
    if use_DampedLeastSquares:
        LocalOpt.Algorithm = (
            self.ZOSAPI.Tools.Optimization.OptimizationAlgorithm.DampedLeastSquares
        )
    else:
        LocalOpt.Algorithm = (
            self.ZOSAPI.Tools.Optimization.OptimizationAlgorithm.OrthogonalDescent
        )
    LocalOpt.Cycles = self.ZOSAPI.Tools.Optimization.OptimizationCycles.Automatic
    LocalOpt.NumberOfCores = int(numCores)
    if self._verbose:
        cp("!@lg!@Solver_LocalOptimization :: Running Local Optimization ...")
    LocalOpt.RunAndWaitForCompletion()
    LocalOpt.Close()
    if self._verbose:
        cp("!@lg!@Solver_LocalOptimization :: Done Local Optimization")
...
def Solver_HammerOptimization(self, secondsRunning: float = 10.0):
    ...
    if self._verbose:
        cp(
            f"!@lg!@Solver_HammerOptimization :: Running Hammer Optimization for [!@lm!@{secondsRunning:.2f}!@g!@] seconds ..."
        )
    ...
    HammerOpt.Close()
    if self._verbose:
        cp("!@lg!@Solver_HammerOptimization :: Done Hammer Optimization.")


######################################################
# Solver Surface Settings
######################################################
def Solver_LDEMakeSurfacePropertyVariable(
    self, in_surface: int | ZOSAPI_Editors_LDE_ILDERow, property: str
):
    """
    ...
    in_surface.GetCellAt(
        int(self.LDE_GetSurfaceColumnEnum(property, in_surface))
    ).MakeSolveVariable()


def Solver_LDEMakeSurfacePropertyFixed(
    ...
    in_surface = self._convert_raw_surface_input_(in_surface, return_index=False)
    in_surface.GetCellAt(
        int(self.LDE_GetSurfaceColumnEnum(propty, in_surface))
    ).MakeSolveFixed()
...
def Solver_GetNamesOfAllSolveTypes(
    ...
    solve_types = list(
        __LowLevelZemaxStringCheck__(
            self, in_obj=self.ZOSAPI.Editors.SolveType, extra_exclude_filter="_"
        )
    )
    if in_surface is not None:
                ...
                for prop in surf_props:
                    cell_solve_data = in_surface.GetCellAt(
                        ...
                        self._CheckIfStringValidInDir_(
                            self.ZOSAPI.Editors.SolveType, solve
                        )
                    )
                    solver_params = [
                        x
                        for x in __LowLevelZemaxStringCheck__(
                            ...
                            self._CheckIfStringValidInDir_(
                                cell_solve_data,
                                "_S_" + solve,
                                extra_exclude_filter="get_",
                            ),
                            extra_exclude_filter="_S_",
                        )
                        if "get_" not in x
                        and "set_" not in x
                        and x != "Type"
        ...
        if print_to_console:
            ...
            for SolveKey in out:
                ...
                for SolveSurface in out[SolveKey]:
...
def Solver_LDESurfaceProperty_ForValue(
    self,
    ...
    CellPropertyCallback = in_surface.GetCellAt(
        int(self.LDE_GetSurfaceColumnEnum(property, in_surface))
    )
    # On that property cell, invoke the solver of type 'value'. I.e. FNumber or Position
    Solver = CellPropertyCallback.CreateSolveType(
        self._CheckIfStringValidInDir_(self.ZOSAPI.Editors.SolveType, solve_type)
    )
    # Get out the '_S_' parameters of that solver (settings of this solver)
    SolverParamCallback = self._CheckIfStringValidInDir_(
        Solver, "_S_" + solve_type, extra_exclude_filter="get_"
    )
    # Set the properties to be solved for (from the params dict).
    for key in params:
        self._SetAttrByStringIfValid_(
            in_obj=SolverParamCallback,
            in_string=key,
            extra_exclude_filter="get_",
            in_value=params[key],
        )
...
##############################################################
def Solver_MCEMakeConfigOpVariable(
    self, in_op: int | ZOSAPI_Editors_MCE_IMCERow, config_number: int
    ...
    solvetype = in_op.GetOperandCell(config_number).CreateSolveType(
        self.ZOSAPI.Editors.SolveType.Variable
    )
...
def Solver_MCEMakeConfigOp_ForValue(
    self,
    ...
    """
    in_op = self._convert_raw_MCEOper_input_(in_op, return_index=False)
    Solver = in_op.GetOperandCell(config_number).CreateSolveType(
        self._CheckIfStringValidInDir_(self.ZOSAPI.Editors.SolveType, solve_type)
    )
    SolverParamCallback = self._CheckIfStringValidInDir_(
        Solver, "_S_" + solve_type, extra_exclude_filter="get_"
