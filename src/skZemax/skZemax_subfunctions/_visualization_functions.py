...
def _Visualization_SEQ_Common_(
    self,
    ...
    myLayout.RaysLineThickness = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.LineThicknessOptions, ray_line_thickness
    )


def Visualization_SEQ_2DCrossSection(
    self,
    ...
    myLayout.ColorRaysBy = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.ColorRaysByCrossSectionOptions, color_rays_by
    )
    myLayout.SurfaceLineThickness = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.LineThicknessOptions, surface_line_thickness
    )
    myLayout.MarginalAndChiefRayOnly = bool(marginal_and_chief_ray_only)
    myLayout.YStretch = float(y_stretch)
    myLayout.Configuration = configuration
    myLayout.RunAndWaitForCompletion()
    myLayout.Close()


def Visualization_SEQ_3DViewer(
    self,
    start_surface_index: int = 1,
    end_surface_index: int = -1,
    wavelength_index: int = -1,
    ...
    myLayout.RayPattern = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.General.RayPatternType, ray_pattern_type
    )
    myLayout.SurfaceLineThickness = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.LineThicknessOptions, surface_line_thickness
    )
    ...
    myLayout.ColorRaysBy = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.ColorRaysByOptions, color_rays_by
    )
    myLayout.DrawRealEntrancePupils = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.RealPupilOptions, draw_real_entrance_pupils
    )
    myLayout.DrawRealExitPupils = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.RealPupilOptions, draw_real_exit_pupils
    )
    myLayout.RunAndWaitForCompletion()
    myLayout.Close()


def Visualization_SEQ_ShadedModel(
    self,
    start_surface_index: int = 1,
    end_surface_index: int = -1,
    wavelength_index: int = -1,
    ...
    myLayout.Opacity = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.OpacityOptions, opacity
    )
    myLayout.Background = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.BackgroundOptions, background
    )
    myLayout.DrawSection = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.DrawSectionOptions, draw_section
    )
    myLayout.AngularSegments = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.NumberSegmentsOptions, angular_segments
    )
    myLayout.RadialSegments = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.NumberSegmentsOptions, radial_segments
    )
    myLayout.Brightness = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.BrightnessOptions, brightness
    )
    myLayout.RunAndWaitForCompletion()
    myLayout.Close()


def _Visualization_NSC_Common_(
    self,
    myLayout: ZOSAPI_Tools_ILayouts,
    save_image: bool,
    saved_image_location: str,
    ...
    myLayout.ConfigurationOffsetY = float(configuration_offset_Y)
    myLayout.ConfigurationOffsetZ = float(configuration_offset_Z)
    myLayout.RayTrace = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.RayTraceOptions, ray_trace_options
    )
    myLayout.ColorRaysBy = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.ColorRaysByNSCOptions, color_rays_by_NSC_options
    )
    myLayout.SurfaceLineThickness = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.LineThicknessOptions, surface_line_thickness
    )
    myLayout.RaysLineThickness = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.LineThicknessOptions, rays_line_thickness
    )


def Visualization_NSC_3DViewer(
    self,
    save_image: bool = True,
    saved_image_location: str = "3DView.png",
    output_pixel_width: int = 3840,
...
def Visualization_NSC_ShadedModel(
    self,
    save_image: bool = True,
    saved_image_location: str = "ShadedModel.png",
    output_pixel_width: int = 3840,
    ...
    myLayout.DetectorPixelColorMode = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.DetectorPixelColorOptions, detector_pixel_color_mode
    )
    myLayout.Background = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.BackgroundOptions, background
    )
    ...
    myLayout.DetectorDisplayMode = self._CheckIfStringValidInDir_(
        self.ZOSAPI.Tools.Layouts.DetectorDisplayModeOptions, detector_display_mode
    )
    myLayout.RunAndWaitForCompletion()
    myLayout.Close()
